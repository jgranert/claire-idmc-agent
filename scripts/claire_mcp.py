...
CACHE_PATH         = os.path.join(_SCRIPT_DIR, ".cache.json")
...
def load_config() -> dict:
    ...
    with open(path) as fh:
        cfg = json.load(fh)
    claire = cfg.get("claireIDMCAgent")
    if not claire:
        log("FATAL: 'claireIDMCAgent' block missing from claude_desktop_config.json")
        sys.exit(1)
    return claire
...
class MCPProxy:
    ...
    async def _list_tools(self, msg_id) -> dict:
        ...
        try:
            ...
            log(f"tools/list ERROR: {exc} — returning empty tool list")
            tools = []
    ...
    async def _handle_initialize(self, msg_id) -> dict:
        ...
        return {
            ...
            "result": {
                ...
                "serverInfo":      {"name": "claire-idmc-agent", "version": "2.0.0"},
            },
        }
